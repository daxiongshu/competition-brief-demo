...
# %% cell: FINAL pure-overlap override (我的改进, 自包含) ===================================
# 重叠井(测试井也在train里)用精确查表 tvt_from_contacts 覆盖最终blend值。
# blend+projection 会把重叠井的精确查表稀释/抹花(投影真值RMSE~1.95); 这里覆盖回精确值(~0.006)。
# 只影响重叠井, 隐藏井不变; 只会更好不会更差。
...
try:
    ...
    _DATA = None
    for _c in [_OvPath("/kaggle/input/competitions/rogii-wellbore-geology-prediction"), _OvPath("/kaggle/input/rogii-wellbore-geology-prediction")]:
        if _c.exists() and (_c/"train").exists(): _DATA=_c; break
    if _DATA is None:
    ...
    _train_wells=set(_ov_os.path.basename(f).split("__")[0] for f in _ov_glob.glob(str(_DATA/"train"/"*__horizontal_well.csv")))
    _n_ov=0
    for _wid,_g in _sub.groupby("well"):
        if _wid not in _train_wells: continue
        try:
            _hw_tr=_ov_pd.read_csv(_DATA/"train"/(_wid+"__horizontal_well.csv"))
            _tw_tr=_ov_pd.read_csv(_DATA/"train"/(_wid+"__typewell.csv"))
            _phys=_ov_np.asarray(_ov_tvt_from_contacts(_hw_tr,_tw_tr),dtype=float)
            for _rid,_ri in zip(_g["id"].astype(str).values,_g["row_idx"].values):
                if 0<=int(_ri)<len(_phys): _pred[_rid]=float(_phys[int(_ri)])
            _n_ov+=1
        except Exception as _e:
            print("overlap override fallback",_wid,_e)
    _sub["tvt"]=_sub["id"].astype(str).map(_pred).astype(float)
    _sub[["id","tvt"]].to_csv(_W/"submission.csv",index=False)
    print("FINAL pure-overlap override applied to",_n_ov,"wells -> submission.csv")
except Exception as _e:
    print("overlap override SKIPPED (kept blend):",_e)
