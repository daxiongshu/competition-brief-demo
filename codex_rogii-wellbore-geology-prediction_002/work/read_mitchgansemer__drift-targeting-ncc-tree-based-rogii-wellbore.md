Reading kernel: mitchgansemer/drift-targeting-ncc-tree-based-rogii-wellbore
Cells: 34 (18 code, 16 markdown) | 59313 chars
...
This notebook explains every key decision in the solution and runs the full     
inference pipeline at the end to reproduce the submission. Each method is shown 
with runnable code immediately after the explanation. Feature generation, model 
training, and Optuna tuning run offline in the pipeline notebooks; those        
sections show the real implementation but skip execution here (~3 hours on a    
laptop).                                                                        
 ...
 ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ 
  ...
  R2         Tree model, 52 features — but absolute TVT    19.50 ft   —         
             target                                                             
  R3         Drift target (TVT − last_anchor_tvt) + GR     14.99 ft   —         
             xcorr + formation KNN                                              
  R4         + Viterbi beam / particle filter sequence     13.96 ft   —         
             estimators                                                         
  R5         Ensemble (LGB + XGB + CatBoost)               13.90 ft   —         
  R6         163 features: multi-scale NCC, Eagle Ford     10.01 ft   9.410 ft  
             formation plane-fit KNN, beam/PF variants;                         
             Optuna tuning; NNLS blend                                          
  R7         CatBoost retuned (depth 5), NNLS blend XGB    10.05 ft   9.398 ft  
             55.9% + CB 44.1%                                                   
  R10        +21 v4 features: estimator divergence,        9.91 ft    —         
             short-window slopes, DWT GR; XGB+CB                                
             retrained                                                          
  R11        + HistGradientBoosting (max_iter=5000, no     9.85 ft    8.905 ft  
             early stopping), NNLS XGB+CB+HGB blend                             
                                                                                

The biggest single jump — R3 — came from reframing the target. Everything after 
that is incremental signal stacking. The three ideas that drive R3→R6 are       
covered in Sections 2–4 below.                                                  

────────────────────────────────────────────────────────────────────────────────
Setup: the rogii-wellbore-models dataset must be attached (it contains the      
pre-trained fold models, KNN structures, and pre-computed ground-truth arrays   
needed below).                                                                  
...
because it drilled straight down, the relationship between depth and GR is      
unambiguous. The horizontal well drills laterally through the same rock, but the
GR at each measured depth maps to an unknown stratigraphic position. The task is
to reconstruct that position (TVT) in the evaluation zone where TVT_input is    
NaN.                                                                            

The anchor zone (rows where TVT_input is not NaN) provides ground truth from a  
geologist's correlation. The evaluation zone is what we predict.                
...
The histograms below show the scale difference. The null model (just predicting 
last_anchor_tvt) achieves 15.91 ft RMSE in TVT space — a surprisingly strong    
baseline because most wells don't drift far from their anchor.                  
...
Gamma ray (GR) measures natural radioactivity: shales are hot (high GR),        
carbonates are cold. The GR trace is a fingerprint of the stratigraphic column —
the same formation always produces the same signal. This means we can find where
the horizontal well is by sliding its GR window against the typewell GR trace   
and finding the best-matching depth.                                            

The function below implements normalized cross-correlation (NCC): it computes   
Pearson r between a short horizontal GR window and each candidate position in   
the typewell, then returns the candidate depth with the highest correlation.    
Multi-scale NCC (half-widths 8, 15, 25 rows) with a softmax blend gives r =     
0.9993 with true TVT across the full training set.                              

The plot shows the full correlation curve at one eval-zone position — a clean   
single peak with a true/estimated TVT separation of only a few feet.            
...
anchor zone where TVT is known.                                                 
...
anchor rows, the GR statistics in the anchor zone, and the depth of the last    
confirmed anchor point. A linear extrapolation from that anchor provides a      
simple but informative prior:                                                   

                                                                                
 tvt_extrap = last_anchor_tvt + tvt_rate × MD_from_anchor                       
                                                                                

The extrapolation RMSE on the example well is large because the well is         
drifting, but it provides an important directional signal that the tree model   
uses to anchor drift predictions near the zone boundary.                        
...
Three additional estimator families add orthogonal drift signals:               
 ...
 • Viterbi beam search (4 variants): an HMM over typewell GR. At each step the  
   most likely TVT position is found by forward-beam Viterbi with varying       
   emission and transition sigmas.                                              
 • Particle filter (3 variants): 500 particles track TVT position through the   
   typewell GR. One variant uses TVT rate, one uses the ANCC plane-fit prior.   

These have large standalone RMSE because they make directional errors, but they 
provide signals orthogonal to the NCC and formation KNN families — the tree     
model learns to use them as drift-direction corrections, not absolute           
predictors.                                                                     
...
163 features are built per-well in 02_features.ipynb via build_features(). The  
function is reproduced in Section 13 (inference pipeline) for transparency —    
it's identical to what runs on Kaggle. The feature groups are:                  
...
same well in both train and val, letting the model memorize GR patterns.        
GroupKFold(5) by well_id ensures that no well appears in both train and val for 
any fold — simulating the unseen-well generalization we need for the hidden test
set.                                                                            
...
Three GBDTs (XGBoost, LightGBM, CatBoost) are trained in 03_modeling.ipynb using
GroupKFold(5). Each fold uses early stopping (5 000 max trees, stop at 50       
non-improving rounds) so the tree count is determined by the data, not the      
hyperparameters. Optuna TPE sampler with 60 initial + 50 warm-start trials per  
model finds the hyperparameters below.                                          
...
OOF predictions use fold models trained on the other 4 folds — each training    
well is predicted by a model that never saw it. This is our primary measure of  
generalization.                                                                 
...
risks overfitting OOF noise. Non-negative least squares (NNLS) blend finds the  
optimal non-negative weights on the OOF — a simple, overfitting-resistant       
approach. HistGradientBoosting uses histogram binning (same inductive bias as   
HGB in scikit-learn), making it structurally different from XGB and CB gradient 
boosters, which is why it earns 36% weight despite weaker solo RMSE.            
...
Savitzky-Golay smoothing: a polynomial smoother applied per-well (window 17,    
order 3) removes row-to-row noise without blurring real geology transitions.    
Applied directly to the blended drift predictions.                              
...
Best LB: 8.905 ft — achieved with R11 (v4 features + HistGradientBoosting, NNLS 
blend XGB 20.8% + CB 43.2% + HGB 36.0%).                                        

v4 / R11: 21 new features (estimator divergence, short-window slopes, DWT GR)   
and adding HistGradientBoosting to the ensemble. OOF 9.85 ft → LB 8.905 ft (gap 
0.94 ft — v4 features generalize strongly to the hidden test distribution).     
...
Absolute TVT target. Predicting raw TVT gave 19.50 ft — 3.6 ft worse than the   
null model. The per-well offset swamps the signal.                              

BiLSTM / TCN. ~14.6 ft OOF. Sequential models memorize per-well GR fingerprints 
rather than learning transferable physics. GroupKFold(5) with 773 wells is not  
enough diversity for a sequence model to generalize.                            

DTW features. Added 14 features but regressed OOF from 10.008 to 10.07 ft. DTW  
uses L2 distance (amplitude-sensitive) while the NCC family uses Pearson r      
(amplitude-invariant). The two metrics are measuring different things; the model
had trouble reconciling them.                                                   

Online learning (all 4 variants, OOF +0.006 to +0.52 ft). Adapting each well's  
model at test time using augmented anchor splits sounds appealing, but the drift
distribution in the eval zone (mean 1.6 ft, std 15.8 ft) is incompatible with   
augmented drift from linspace anchor splits (spanning hundreds of feet). Early  
stopping collapsed toward the augmented distribution rather than the true one.  

ExtraTrees. Solo RMSE 10.89 ft (weaker than HGB's 10.02 ft), and training on    
3.8M rows without histogram binning takes hours even with n_jobs=-1. HGB covers 
the same structural diversity with a fraction of the compute.                   

Coordinate-overlap post-processing. Only 12/773 training wells share coordinates
with any other training wells. The reference notebook's reported gain appears   
only because their visible test wells are training wells (100% overlap). For    
unseen test wells, median overlap is 0.00% — no generalizable benefit.          
...
fold model prediction, NNLS blend, post-processing, and submission.csv output.  
This is identical to the code in the production inference kernel — the same     
artifacts from rogii-wellbore-models are used.                                  

On Kaggle (~200 test wells): ~20 minutes for feature building, ~2 minutes for   
model inference. Locally (3 visible test wells): ~3 minutes total.              
