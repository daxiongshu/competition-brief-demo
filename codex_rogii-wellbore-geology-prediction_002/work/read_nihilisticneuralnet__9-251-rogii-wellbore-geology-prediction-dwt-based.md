...
Cells: 24 (15 code, 9 markdown) | 51575 chars
...
────────────────────────────────────────────────────────────────────────────────
     ...
     for step in range(n):                                                      
         ...
         for bi in range(bn):                                                   
             ...
             for d in range(-2, 3):                                             
                 ...
                 tot = cost + (gv - tw_gr[ni]) ** 2 / es + mc * (d if d >= 0 el 
 -d)                                                                            
                 fnd = np.int64(-1)                                             
                 for ci in range(nc):                                           
 ...
 def _dtw_stochastic_realizations(query, ref, radius, K, temperature):          
     ...
     for k in range(K):                                                         
         ...
         for i in range(N):                                                     
             ...
             j_hi = min(M - 1, j_center + radius)                               
             for j in range(j_lo, j_hi + 1):                                    
                 noise = -temperature * np.log(-np.log(np.random.uniform(1e-10, 
 1.0)))                                                                         
 ...
 def _pf_ancc(md_v, z_v, gr_v, gg, vmin, step, gs, ls, ir, N,                   
     ...
     for i in range(len(md_v)):                                                 
         ...
         for j in range(N):                                                     
             ...
             tvt_j = pos[j] - z_v[i]                                            
             tvt_j = max(tvt_j, vmin - 50.); tvt_j = min(tvt_j, vmin + len(gg)  
 step + 50.)                                                                    
             pos[j] = tvt_j + z_v[i]                                            
 ...
 = z_v[0] - 1.                                                                  
     for i in range(len(md_v)):                                                 
         ...
         for j in range(N):                                                     
             ...
             pos[j] = max(pos[j], vmin - 50.); pos[j] = min(pos[j], vmin +      
 len(gg_p) * step + 50.)                                                        
         if not np.isnan(gr_v[i]):                                              
             ws = 0.                                                            
             for j in range(N):                                                 
                 ...
                 dp = (gr_v[i] - ep) / gs                                       
                 lp = max(np.exp(-0.5 * dp * dp) if dp * dp < 600. else 0.,     
 1e-300)                                                                        
                 if not np.isnan(gr_sm_v[i]):                                   
                     es = _interp1(gg_s, pos[j], vmin, step)                    
                     ds = (gr_sm_v[i] - es) / (gs * 1.5)                        
                     ls = max(np.exp(-0.5 * ds * ds) if ds * ds < 600. else 0., 
 1e-300)                                                                        
 ...
 def _gr_sig(hw, tw_tvt, tw_gr):                                                
     ...
     return float(np.clip(np.std(kn['GR'].values -                              
 ...
 dtype='float32').interpolate(limit_direction='both').fillna(fb)                
     return (s.rolling(r * 2 + 1, center=True, min_periods=1).mean() if r > 0   
 else s).to_numpy(np.float32)                                                   
                                                                                
                                                                                
 def beam_search(gr_h, tw_tvt, tw_gr, start_tvt, bs, mc, es, r):                
     si = _nn(tw_tvt, start_tvt)                                                
     sgr = _smooth(gr_h, float(np.nanmean(tw_gr)), r).astype(np.float64)        
     path = _beam_jit(sgr, tw_gr.astype(np.float64), si, bs, float(mc),         
 float(es))                                                                     
 ...
 def run_pf_ancc(hw, tw_tvt, tw_gr, N=ANCC_N):                                  
     ...
     dz = np.diff(tail['Z'].values); dm = np.diff(tail['MD'].values); m = dm >  
 ...
 ev['Z'].values.astype(np.float64),                                             
                         ...
                         gs, ls, ir, N, ANCC_ALPHA, ANCC_RN, ANCC_PN, ANCC_IS,  
 ANCC_RP, ANCC_RR, PF_RESAMP)                                                   
     return pts.astype(np.float32), std.astype(np.float32)                      
                                                                                
                                                                                
 def run_pf_z(hw, tw_tvt, tw_gr, N=PF_N):                                       
     gs = _gr_sig(hw, tw_tvt, tw_gr)                                            
     tw_s = pd.Series(tw_gr).rolling(PF_GR_WIN, center=True,                    
 min_periods=1).mean().values.astype(np.float32)                                
     ...
     if m2.sum() >= 10:                                                         
         ...
         A = np.column_stack([vz, np.ones_like(vz)]); c, _, _, _ =              
 np.linalg.lstsq(A, vt, rcond=None)                                             
         beta, icpt, zsig = float(c[0]), float(c[1]), max(float(np.std(vt - (c[ 
 * vz + c[1]))), 0.001)                                                         
     else:                                                                      
 ...
 ev['Z'].values.astype(np.float64),                                             
                      ...
                      gg, gs2, gmin, gst, gs, float(kna['TVT_input'].iloc[-1]), 
 iv,                                                                            
                      beta, icpt, zsig, N,                                      
 ...
 def run_dtw_multiscale(query_gr, tw_tvt, tw_gr, last_tvt, radii=DTW_RADII):    
     ...
     For each Sakoe-Chiba radius, runs DTW and maps the warping path back to TV 
 space.                                                                         
 ...
 len(rn_f), 1)                                                                  
         tvt_pred = _dtw_path_to_tvt(pi[::-1], pj[::-1],                        
 tw_tvt.astype(np.float32), N)                                                  
     ...
     weights = np.array([ic / inv_cost_sum for _, ic in tvt_stack],             
 dtype=np.float32)                                                              
 ...
 def run_dtw_stochastic(query_gr, tw_tvt, tw_gr, last_tvt,                      
     ...
     """                                                                        
     N = len(query_gr)                                                          
     qn = ((query_gr - query_gr.mean()) / (query_gr.std() +                     
 ...
 _md = np.linspace(1, 50, 20, np.float64); _z = np.zeros(20, np.float64); _gr = 
 np.full(20, 50., np.float64)                                                   
 _gg = np.linspace(45, 55, 100, np.float64)                                     
 _pf_ancc(_md, _z, _gr, _gg, 45., 0.1, 20., 50., 0., 8, 0.998, 0.002, 0.005, 0. 
 0.1, 0.001, 0.5)                                                               
 ...
 def affine_cal(kgr, tw_at_k, min_pts=20):                                      
     ...
     if v.sum() < min_pts or np.std(tw_at_k[v]) < 1e-6:                         
         return 1., float(np.nanmean(kgr) - np.nanmean(tw_at_k)) if v.any() els 
 0.                                                                             
 ...
 np.inf, dist)                                                                  
         ...
         dk = np.take_along_axis(dist, ord, 1); ik = np.take_along_axis(idx, or 
 1)                                                                             
 ...
 w * yn                                                                         
         ...
         A[:, 0, 0] = (wx * xn).sum(1); A[:, 0, 1] = (wx * yn).sum(1); A[:, 0,  
 = wx.sum(1)                                                                    
         A[:, 1, 0] = A[:, 0, 1]; A[:, 1, 1] = (wy * yn).sum(1); A[:, 1, 2] =   
 wy.sum(1)                                                                      
         A[:, 2, 0] = A[:, 0, 2]; A[:, 2, 1] = A[:, 1, 2]; A[:, 2, 2] = w.sum(1 
         A[:, 0, 0] += 1e-9; A[:, 1, 1] += 1e-9; A[:, 2, 2] += 1e-9             
         rhs = np.stack([(wx[:, :, None] * fn).sum(1), (wy[:, :, None] *        
 fn).sum(1),                                                                    
                         (w[:, :, None] * fn).sum(1)], 1)                       
         try:                                                                   
         ...
         Xq = xy_q[:, 0]; Yq = xy_q[:, 1]                                       
         pred = (Xq[:, None] * coef[:, 0, :] + Yq[:, None] * coef[:, 1, :] +    
 ...
 df.iloc[ix]                                                                    
         ...
         self.ancc = np.concatenate(anccs).astype(np.float32); self.wids =      
 np.array(wids)                                                                 
 ...
 1)                                                                             
         ...
         ap = (an * w).sum(1) / safe; ap = np.where(sw < 1e-9,                  
 ...
 _FI = FI; _DI = DI                                                             
 ANCH_OFFS = np.array([-80, -40, -20, -10, -5, 0, 5, 10, 20, 40, 80], np.float3 
 ...
 def build_well(hw_path, tw_path, is_train):                                    
     ...
     tw_tvt = tw['TVT'].to_numpy(np.float32); tw_gr =                           
 tw['GR'].to_numpy(np.float32)                                                  
     ...
     gr_full =                                                                  
 hw['GR'].astype(float).interpolate(limit_direction='both').fillna(float(np.nan 
 an(tw_gr)))                                                                    
 ...
 1).mean(1).astype(np.float32)                                                  
     dtw_ens_slope_ev = np.stack([dtw_slope_ev[r] for r in DTW_RADII],          
 1).mean(1).astype(np.float32)                                                  
 ...
 ktvt[-50:])                                                                    
     ...
     xy_ev = ev[['X', 'Y']].to_numpy(np.float64); xy_kn = kn[['X',              
 ...
 gr_s.shift(-lag).ffill().iloc[ev.index].values.astype(np.float32)              
     ...
     gr_d2 =                                                                    
 gr_s.diff().diff().fillna(0.).iloc[ev.index].values.astype(np.float32)         
     gr_env = gr_s.rolling(21, center=True,                                     
 min_periods=1).max().iloc[ev.index].values.astype(np.float32)                  
     gr_nrg = np.sqrt(np.maximum((gr_s ** 2).rolling(21, center=True,           
 ...
 in bpaths.items()},                                                            
         'beam_mean_d': np.stack([(p - last_tvt) for p in bpaths.values()],     
 1).mean(1).astype(np.float32),                                                 
         'beam_std_d': np.stack([(p - last_tvt) for p in bpaths.values()],      
 1).std(1).astype(np.float32),                                                  
         'beam_med_d': np.median(np.stack([(p - last_tvt) for p in              
 bpaths.values()], 1), 1).astype(np.float32),                                   
         'sc8_d': (sc8 - np.float32(last_tvt)).astype(np.float32), 'sc8_sc':    
 sc8s,                                                                          
 ...
 sc25s,                                                                         
         ...
         'sc_trust': sc(sc_trust), 'hyb_d': (hyb_ref -                          
 np.float32(last_tvt)).astype(np.float32),                                      
         'sig_std': sig_std, 'sig_mean_d': sig_mean,                            
         **tvt_fs,                                                              
         **{f'frm_rmse_{fn}': sc(form_rmse[fn]) for fn in FORMATIONS},          
         'form_mean_d': form_mean_d, 'form_std_d': form_std_d, 'form_rng_d':    
 form_rng_d,                                                                    
         'spatial_ancc_d': (form_ev[:, 0] - np.float32(np.interp(last_tvt,      
 tw_tvt, tw_gr))),                                                              
 ...
 sc(d_nb_std),                                                                  
         ...
         'spatial_vs_dense': (tvt_fs['tvtF_ANCC'] -                             
 tvt_dense).astype(np.float32),                                                 
         'beam_vs_spatial': (bpaths['cons'] -                                   
 tvt_fs['tvtF_ANCC']).astype(np.float32),                                       
         ...
         **{f'dtw_r{r}_d': (dtw_per_radius_ev[r] - last_tvt).astype(np.float32) 
 for r in DTW_RADII},                                                           
         ...
         **{f'tddtw{int(o)}': hgr - np.interp(dtw_ens_ev + o, tw_tvt,           
 tw_gr).astype(np.float32)                                                      
            for o in DTW_OFFS},                                                 
         ...
         'ktvt_range': sc(float(np.ptp(ktvt))), 'ktvt_std':                     
 sc(float(ktvt.std())),                                                         
         'md_since': md_since, 'frac': frac, 'frac2': frac ** 2, 'sqrt_frac':   
 np.sqrt(frac),                                                                 
         'z': z_ev,                                                             
         'dx': (ev['X'] - float(lk['X'])).to_numpy(np.float32),                 
         'dy': (ev['Y'] - float(lk['Y'])).to_numpy(np.float32),                 
         'dz': (z_ev - float(lk['Z'])).astype(np.float32),                      
         'dxy': np.sqrt((ev['X'] - float(lk['X'])) ** 2 + (ev['Y'] -            
 float(lk['Y'])) ** 2).to_numpy(np.float32),                                    
         'dzdmd': dzdmd, 'dxdmd': dxdmd, 'dydmd': dydmd,                        
         'gr': hgr, 'gr_d1': gr_d1, 'gr_d2': gr_d2, 'gr_env': gr_env, 'gr_nrg': 
 gr_nrg,                                                                        
         'gr_vs_tw_anc': hgr - np.float32(np.interp(last_tvt, tw_tvt, tw_gr)),  
         'gr_vs_slp_all': hgr - np.interp(slp_b_all, tw_tvt,                    
 tw_gr).astype(np.float32),                                                     
         **{f'tda{int(o)}': hgr - np.float32(np.interp(last_tvt + o, tw_tvt,    
 tw_gr)) for o in ANCH_OFFS},                                                   
         **{f'tdbc{int(o)}': hgr - np.interp(beam_ref + o, tw_tvt,              
 tw_gr).astype(np.float32) for o in BEAM_OFFS},                                 
         **{f'tdsc{int(o)}': hgr - np.interp(sc_ens + o, tw_tvt,                
 tw_gr).astype(np.float32) for o in SC_OFFS},                                   
         **{f'tdpf{int(o)}': hgr - np.interp(pf_use + o, tw_tvt,                
 tw_gr).astype(np.float32) for o in PF_OFFS},                                   
         'tw_range': sc(float(np.ptp(tw_tvt))), 'tw_gr_mean':                   
 sc(float(tw_gr.mean())),                                                       
     }                                                                          
     ...
     if is_train:                                                               
         ...
         result['target'] = (ev['TVT'].to_numpy(np.float32) -                   
 np.float32(last_tvt))                                                          
     return result                                                              
                                                                                
                                                                                
 def build_dataset(paths, is_train, label):                                     
     args = [(str(p), str(p.parent / f'{p.stem.replace("__horizontal_well",     
 "")}__typewell.csv'), is_train)                                                
             for p in paths                                                     
...
────────────────────────────────────────────────────────────────────────────────
 ...
 num_iteration=m.best_iteration).astype(np.float32) / CFG.n_splits              
             score = root_mean_squared_error(y.iloc[valid_idx],                 
 oof_preds[valid_idx])                                                          
     ...
     for fold_idx, (train_idx, valid_idx) in enumerate(splits):                 
         ...
         d_valid = lgb.Dataset(X.iloc[valid_idx], label=y.iloc[valid_idx],      
 reference=d_train)                                                             
         m = lgb.train(params, d_train, valid_sets=[d_valid],                   
 ...
 for i in range(3):                                                             
     _oof_preds, _test_preds, _overall_score, _fold_scores =                    
 train_lightgbm(lgb_params[i], f"lightgbm-{i + 1}")                             
...
────────────────────────────────────────────────────────────────────────────────
 ...
 oof_preds[valid_idx])                                                          
     ...
     for fold_idx, (train_idx, valid_idx) in enumerate(splits):                 
         ...
         m.fit(Pool(X.iloc[train_idx].values, label=y.iloc[train_idx].values),  
               eval_set=Pool(X.iloc[valid_idx].values,                          
 ...
 for i in range(3):                                                             
     _oof_preds, _test_preds, _overall_score, _fold_scores =                    
 train_catboost(cb_params[i], f"catboost-{i + 1}")                              
...
────────────────────────────────────────────────────────────────────────────────
 ...
 sub['tvt'] = sub['tvt'].fillna(float(train_df['last_known_tvt'].mean() +       
